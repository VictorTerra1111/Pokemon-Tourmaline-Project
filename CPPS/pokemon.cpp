...
bool Pokemon::canevolve(){
    ...
    if(legendary || mythical) return false;
    if (stage == 2) return false;
    if(xp > nec) return false;
...
void Pokemon::shinycalculate(){
    ...
    if(poke_odd == 0) shiny = true;

    else shiny = false;
}

bool Pokemon::islegendary(string namepkmn){
    if(namepkmn == "Veracton" || namepkmn == "Lavorus" || namepkmn == "Danterius") return true;
    return false;
}

bool Pokemon::ismythical(string namepkmn){
    if (namepkmn == "Witnelle"){ return true;}
    return false;
}
